...
def translate_to_fitch(rating: str) -> Optional[str]:
    ...
    # Try Moody's first (can contain numbers like Baa1, Aa2, etc.)
    if rating_upper in moodys_to_fitch:
        return moodys_to_fitch[rating_upper]
    
    # Try S&P (contains +/- but no numbers in the middle)
    if rating_upper in sp_to_fitch:
        return sp_to_fitch[rating_upper]
    
    # Check if it's already a valid Fitch rating (just needs normalization)
    if rating_upper in valid_fitch:
        return rating_upper
